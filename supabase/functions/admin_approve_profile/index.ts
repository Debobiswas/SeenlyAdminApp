...
async function getAdminUserId(req: Request): Promise<string | Response> {
  ...
  if (adminErr) {
    console.error("admin_approve_profile admin lookup error", adminErr);
...
serve(async (req: Request) => {
  ...
  let payload: { target_user_id?: unknown };
  try {
  ...
  if (updateErr) {
    console.error("admin_approve_profile update error", updateErr);
    return json({ error: "Profile approval failed" }, 500);
  }
  ...
  if (logErr) {
    console.error("admin_approve_profile audit log error", logErr);
